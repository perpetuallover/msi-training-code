...
//		$http.delete("rest/person/" + emp.name).then(function(resp){
//			$scope.empList.splice(index, 1);
//		});
		$http({
			url: "rest/person/" + emp.name,
			method: "DELETE"
		}).then(function(resp){
			$scope.empList.splice(index, 1);
		})
...
app.controller("addCtrl", ["$scope", "$http", "$location", function($scope, $http, $location){
	$scope.doSubmit = function(){
		$http.post("rest/person", $scope.emp).then(function(resp){
			console.log("resp");
			$location.path("/home");
		})
	}
...
app.controller("updateCtrl", ["$scope", "$http", "$location", "$routeParams", "empFactory",
	function($scope, $http, $location, $routeParams, empFactory){
		...
		$http.get('rest/person/' + name).then(function(resp){
			$scope.emp  = resp.data;
			$scope.originalEmp = angular.copy($scope.emp);
		});
		$scope.doSubmit = function(){
			$http.put("rest/person", $scope.emp).then(function(resp){
				console.log(resp);
				$location.path("/home");
			})
		}
...
app.factory("empFactory", function($q, $http){
	return {
		getEmpList : function(){
			var defer = $q.defer();
			$http.get("rest/person").then(function(resp){
